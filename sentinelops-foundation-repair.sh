...
if [ ! -d apps/web/app ] || [ ! -f docker-compose.yml ]; then
  echo "Run this from the SentinelOps project root."
  exit 1
fi

mkdir -p .repair-backup
cp apps/web/app/settings/page.tsx ".repair-backup/settings.page.$(date +%Y%m%d%H%M%S).tsx" 2>/dev/null || true
cp docker-compose.yml ".repair-backup/docker-compose.$(date +%Y%m%d%H%M%S).yml" 2>/dev/null || true

cat > apps/web/app/settings/page.tsx <<'TSX'
'use client';
...
s = p.read_text()
s = s.replace('POSTGRES_DB: ${POSTGRES_DB:-sentinelops}', 'POSTGRES_DB: ${POSTGRES_DB}')
s = s.replace('POSTGRES_USER: ${POSTGRES_USER:-sentinelops}', 'POSTGRES_USER: ${POSTGRES_USER}')
s = s.replace('POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-sentinelops}', 'POSTGRES_PASSWORD: ${POSTGRES_PASSWORD}')
s = s.replace('pg_isready -U ${POSTGRES_USER:-sentinelops} -d ${POSTGRES_DB:-sentinelops}', 'pg_isready -U ${POSTGRES_USER} -d ${POSTGRES_DB}')
s = s.replace('NEXT_PUBLIC_API_BASE_URL: ${NEXT_PUBLIC_API_BASE_URL:-http://localhost:8080/api/v1}', 'NEXT_PUBLIC_API_BASE_URL: ${NEXT_PUBLIC_API_BASE_URL}')
needed = [
    ...
    'SENTINELOPS_BOOTSTRAP_TOKEN: ${SENTINELOPS_BOOTSTRAP_TOKEN}',
]
if 'INTEGRATION_ENCRYPTION_KEY: ${INTEGRATION_ENCRYPTION_KEY}' in s:
    anchor = 'INTEGRATION_ENCRYPTION_KEY: ${INTEGRATION_ENCRYPTION_KEY}'
    block = anchor + '\n' + '\n'.join('      ' + x for x in needed)
    # Avoid duplicating if they were already applied.
    if 'CORS_ALLOWED_ORIGINS: ${CORS_ALLOWED_ORIGINS}' not in s:
        s = s.replace(anchor, block)
p.write_text(s)
PY

rm -f apps/web/app/settings/page.tsx.rej docker-compose.yml.rej

echo "Repair applied. Backups saved under .repair-backup/."
